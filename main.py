...
from tkinter import Scale, StringVar, Radiobutton, Button, Frame

def parse_args(gui_args):
    parser = argparse.ArgumentParser(description='Run the snake game in different modes.')
    
    parser.add_argument('--mode', default='rl', choices=['rl', 'search', 'human'], type=str, help='Mode to run the game: "rl" for reinforcement learning, "search" for search-based AI, or "human" for playing manually.')
    parser.add_argument('--learning_rate', default=0.00025, type=float, help='Learning rate for the RL agent.')
    parser.add_argument('--discount_factor', default=0.9, type=float, help='Discount factor for the RL agent.')
    parser.add_argument('--hidden_layers', default=1, type=int, help='Number of hidden layers for the neural network.')
    parser.add_argument('--epsilon', default=1.0, type=float, help='Initial epsilon value for the epsilon-greedy strategy.')
    parser.add_argument('--epsilon_decay', default=0.995, type=float, help='Epsilon decay rate per episode.')
    parser.add_argument('--epsilon_min', default=0.001, type=float, help='Minimum epsilon value.')
    
    return parser.parse_args()

def snake_game(mode, learning_rate, discount_factor, hidden_layers, epsilon, epsilon_decay, epsilon_min):
    env = Environment()
    if mode.lower() == 'rl':
...
def start_gui():
    def run_game():
        ...
        snake_game(mode, learning_rate, discount_factor, hidden_layers, epsilon, epsilon_decay, epsilon_min)
    ...
    for i in range(10):  # Adjust the range based on the number of rows needed
